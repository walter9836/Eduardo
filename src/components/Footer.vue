      ...
      <div class="container mx-auto px-4">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <ul>
              <li class="mb-2"><a href="/about" class="hover:underline">Acerca de Nosotros</a></li>
  ...
  const validateEmail = () => {
    ...
    const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    emailError.value = regex.test(email.value) || email.value === '' ? '' : '';
  };

  // Función para manejar la suscripción al boletín
  const subscribe = async () => {
    // Verifica que el email no esté vacío y sea válido
    if (email.value && !emailError.value) {
      isSubmitting.value = true; // Activar el estado de carga

      try {
        // Enviar la solicitud POST al backend (ejemplo con WordPress)
        const response = await axios.post('https://dev-edu123t35.pantheonsite.io/wp-json/sb/v1/suscribir', {
          email: email.value,
        });

        // Manejo de la respuesta según el mensaje recibido
        if (response.data === 'Este correo ya está registrado.') {
          statusMessage.value = 'Este correo ya está registrado. ¡Gracias!';
          statusClass.value = 'text-yellow-500'; // Color amarillo para "ya registrado"
        } else {
        ...
        isSubmitting.value = false; // Desactivar el estado de carga
      }
    } else {
